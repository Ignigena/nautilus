...
function NautilusCorePolicies (app) {
  ...
  const defaultPolicy = app.config.policies.default || app.config.policies['*']
...
function whitelistCheck (policy) {
  return function (req, res, next) {
    ...
    if (policy.args && !Array.isArray(policy.args)) {
      policy.args = [policy.args]
    }

    const fn = (policy.fn || req.app.api.policies[policy.name])(req, res, next)
    return (policy.args && fn(...policy.args)) || fn
  }
}

function whitelistPass (originalUrl) {
  originalUrl = url.parse(originalUrl).pathname
  return middlewareWhitelist.some(path => originalUrl.match(match(path)))
}
