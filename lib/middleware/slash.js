...
/**
 * Determine whether or not trailing slashes should be removed or enforced (the
 * default behaviour.) To strip trailing slashes set `app.config.slash` or
 * `app.config.slash.trailing` to `false`. This allows whitelisting in either
 * configuration.
...
function shouldRemoveTrailingSlashes(config) {
  if (typeof config.slash === 'undefined') return false;
  return (config.slash === false || config.slash.trailing === false);
}
