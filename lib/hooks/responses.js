...
let responseCodes = _.reduce(httpStatusCodes, (result, code) => {
...
module.exports = function responsesHook(app) {
  ...
  app.hooks.after('core:views', () => {
    _.each(responseCodes, (statusText, status) => {
      statusText = changeCase.paramCase(statusText);
      let path = this.responsePath(statusText);
  ...
  app.response.negotiate = function(view, body) {
    ...
    this.format({
      text: () => {
        this[typeof body === 'object' ? 'json' : 'send'](body);
      },
