...
module.exports = function responsesHook(app) {
  ...
  // When in development, all JSON responses are returned formatted.
  ...
  app.events.once('hooks:loaded:core:views', () => {
    viewPaths = {
  ...
  // res.unauthorized()
  // --
  // Sends a 401 "Unauthorized" response to the client indicating the server
  // requires authorization but none was provided.
  app.response.unauthorized = function(body) {
    this.status(401);
    if (viewPaths[401]) return this.negotiate(viewPaths[401], body);
    this.negotiate(body);
  };
