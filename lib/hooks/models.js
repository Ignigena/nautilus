...
function NautilusCoreORMHook(app) {
  ...
  _.each(app.api.model, function(settings, model) {
    let modelName = changeCase.pascal(model);
    ...
    // Virtuals can be defined on the `virtual` property of the model. If only a
    // getter is needed a single function can be passed. Otherwise, a `set` and
    // `get` key can be specified with their respective functions.
    ...
    _.each(settings.middleware, function(hook, type) {
      ...
      let what = hookDetails[2].toLowerCase();
